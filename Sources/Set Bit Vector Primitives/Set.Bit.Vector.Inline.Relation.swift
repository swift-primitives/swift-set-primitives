...
extension Set<Bit>.Vector.Static.Relation {
    ...
    @inlinable
    public func isSubset(of other: Set<Bit>.Vector.Static<wordCount>) -> Bool {
    ...
    @inlinable
    public func isSuperset(of other: Set<Bit>.Vector.Static<wordCount>) -> Bool {
        other.relation.isSubset(of: Set<Bit>.Vector.Static<wordCount>(__storage: storage))
    }
    ...
    @inlinable
    public func isDisjoint(with other: Set<Bit>.Vector.Static<wordCount>) -> Bool {
        for i in 0..<wordCount {
            if (storage[i] & other.storage[i]) != 0 {
                return false
            }
