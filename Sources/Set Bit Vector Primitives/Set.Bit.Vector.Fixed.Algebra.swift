...
extension Set<Bit>.Vector.Fixed.Algebra {
    ...
    @inlinable
    public func union(_ other: Set<Bit>.Vector.Fixed) -> Set<Bit>.Vector.Fixed {
        ...
        return Set<Bit>.Vector.Fixed(__storage: resultStorage, capacity: capacity)
    }
    ...
    @inlinable
    public func intersection(_ other: Set<Bit>.Vector.Fixed) -> Set<Bit>.Vector.Fixed {
        precondition(capacity == other.capacity, "Capacities must match")
        var resultStorage = storage
        for i in 0..<resultStorage.count {
            resultStorage[i] &= other.storage[i]
    ...
    @inlinable
    public func subtract(_ other: Set<Bit>.Vector.Fixed) -> Set<Bit>.Vector.Fixed {
        precondition(capacity == other.capacity, "Capacities must match")
        var resultStorage = storage
        for i in 0..<resultStorage.count {
            resultStorage[i] &= ~other.storage[i]
...
extension Set<Bit>.Vector.Fixed {
    ...
    @inlinable
    public mutating func form(_ operation: (Algebra) -> Set<Bit>.Vector.Fixed) {
